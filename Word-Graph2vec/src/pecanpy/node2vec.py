...
class Base:
    ...
    def simulate_walks(self, num_walks, walk_length):
        ...
        indptr = self.indptr
        sum_degree = indptr[num_nodes]
        nodes_walk = []
        import math
        for i in range(0, num_nodes):
            nodes_walk.append(5 + math.ceil(num_nodes * num_walks * ((indptr[i + 1] - indptr[i]) / sum_degree)))
        start_node_idx_ary = np.array([])
        for i in range(0,num_nodes):
            temp = np.array([nodes[i]] * int(nodes_walk[i]))
            start_node_idx_ary = np.concatenate((start_node_idx_ary,temp))
